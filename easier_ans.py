...
DEFAULT_MODEL = os.getenv("OLLAMA_MODEL", "gemma3:12b") # User might want to try a smaller model too for speed/less complexity
...
#__________________________________________________________________________________________
    # --- USER INPUTS ---
DEFAULT_RESUME_FILENAME = 'kenji_gamer_resume.pdf' # Example filename, will be highlighted in main()
#__________________________________________________________________________________________
...
class MainCharacterChain:
    ...
    def run(self, file_name, genre):
        try:
            ...
            if not profile or profile.startswith("Error:") or len(profile) < 100:
    ...
    **Focus on attributes that lead to engaging and understandable plot developments.**
    ...
    def run(self, subject, genre, author, profile, title, setting, themes_str):
        try:
            ...
            if not plot or plot.startswith("Error:") or len(plot) < 200:
...
class ChaptersChain:
    ...
    **Chapter descriptions should be concise, clear, and hint at concrete actions or revelations.**
    ...
    def parse(self, response):
        chapters = {}
        lines = [line.strip() for line in response.strip().split('\n') if line.strip()]
        # print(f"\nRaw Chapter Response Lines ({len(lines)}):") # Uncomment for deep debug
        ...
        for match in matches:
            ...
            if description:
                ...
                for i, line in enumerate(lines):
                    if raw_line_approx in line:
                         processed_lines.add(i)
                         break
            else:
                print(f"  Warning: Skipping match - Empty description: '{match[0]}'")

        # print(f"Attempting fallback parsing for remaining lines...") # Uncomment for deep debug
        for i, line in enumerate(lines):
        ...
        return dict(sort_chapters(chapters))
...
class WriterChain:
    ...
    **WRITING INSTRUCTIONS (CRITICAL):**
    ...
    def __init__(self):
        # FURTHER REDUCED temperature and top_p for more focused and potentially simpler prose.
        self.llm = create_llm(temperature=0.7, top_p=0.85, top_k=40)
        self.chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplate.from_template(self.PROMPT),
            verbose=True
    ...
    def run(self, genre, author, title, profile, plot, setting, themes_str, chapter_name,
        ...
        try:
            print(f"Invoking WriterChain for event: {current_event[:80]}...")
            time.sleep(LLM_CALL_DELAY_SECONDS)
            result = self.chain.invoke({
                "genre": genre,
                "author": author,
            ...
            if not generated_text or generated_text.startswith("[ERROR:") or len(generated_text) < 50:
...
# --- Refinement Chain (Optional Post-Processing) ---
class RefinementChain:
    ...
    **YOUR TASK:**
    ...
    1.  **Stylistic Consistency:** Enhance adherence to {author}'s unique voice, sentence structure, and vocabulary, **while maintaining clarity.**
    2.  **Prose Flow & Pacing:** Improve readability, sentence variety, and transitions between paragraphs. Smooth out awkward phrasing.
    3.  **Sensory Detail Enhancement:** Weave in more vivid and **relatable** sensory details based on the setting.
    4.  **Dialogue Polish:** Make dialogue sound more natural and character-specific (if applicable).
    5.  **Thematic Resonance:** Subtly strengthen connections to the core themes ({themes}) **without becoming overly abstract.**
    6.  **Show, Don't Tell:** Convert any instances of telling into **concrete showing** through action, internal thought, or description.
    7.  **Conciseness:** Trim unnecessary words or redundant phrases without losing meaning or style.
    8.  **Enhance Clarity & Relatability:** If the draft seems dense or uses overly complex language, simplify sentences and ground abstract descriptions with more concrete details or relatable comparisons. Ensure the reader can easily follow the narrative and connect with the character's experience.
    ...
    **INSTRUCTIONS:**
    ...
    def run(self, author, title, genre, profile, setting, themes_str, plot, chapter_name, summary, draft_text):
        ...
        try:
            ...
            if not refined_text or refined_text.startswith("[ERROR:") or len(refined_text) < len(draft_text) * 0.5:
    ...
    def chapter_sort_key(chapter_title):
        ...
        if match:
            try:
                ...
                return (2, chapter_title)
        return (3, chapter_title)
...
def write_book(genre, author, title, profile, plot, setting, themes_str, chapter_dict, event_dict, refine_chapters=False):
    ...
    for chap_idx, (chapter_title, chapter_summary) in enumerate(sorted_chapter_items):
        ...
        for event_idx, event_description in enumerate(chapter_events):
            ...
            print(f"   Writing Event {event_idx+1}/{total_events}: {event_description[:100]}...")
...
class DocWriter:
    ...
    def write_doc(self, book_content, chapter_dict, title, genre, author, themes_dict, setting):
        ...
        themes_str_val = format_themes_string(themes_dict) # Renamed to avoid conflict
        if themes_str_val == "N/A" or "Error" in themes_dict:
...
def main():
    ...
    print("=============================================")
    print("=== ENHANCED NOVEL GENERATION SYSTEM V2.2 ===") # Version bump for clarity
...
#__________________________________________________________________________________________
    # --- USER INPUTS ---
    # Ensure the resume PDF (e.g., DEFAULT_RESUME_FILENAME) is in the OUTPUT_FOLDER (./docs/)
    # or update DEFAULT_RESUME_FILENAME at the top of the script, or the 'resume_filename' variable below.

    resume_filename = DEFAULT_RESUME_FILENAME # Or specify a different filename here: e.g., "my_actual_resume.pdf"
...
#__________________________________________________________________________________________
    ...
    profile = "Error: Profile generation skipped."
    setting = "Error: Setting generation skipped."
    themes_dict = {"Error": "Theme generation skipped."}
    themes_str = "N/A"
    title = "Error - Title Generation Failed"
    plot = "Error: Plot generation skipped."
    chapter_dict = {}
    event_dict = {}
    book_content = {}
    ...
    try:
        ...
        if generation_successful:
            ...
            if setting.startswith("Error:"):
                ...
                # generation_successful = False # Not critical enough to stop all
            else:
        ...
        if generation_successful:
            ...
            if "Error" in themes_dict:
                print(f"THEME GENERATION/PARSING FAILED: {themes_dict.get('Error')}")
            else:
        ...
        if generation_successful:
            ...
            if not chapter_dict:
                 ...
                 for i, (chap_title, chap_desc) in enumerate(chapter_dict.items()):
                      if i < 5: print(f"  - {chap_title}: {chap_desc}") # Print first 5
                 if len(chapter_dict) > 5: print(f"  ... and {len(chapter_dict)-5} more chapters.")
        ...
        if generation_successful:
            ...
            if not any(event_dict.values()):
                print("CRITICAL WARNING: Failed to generate events for *any* chapter.")
...
if __name__ == "__main__":
    ...
    # Example: `ollama run llama3` or `ollama run gemma2` in your terminal.
    # Ensure the resume PDF is in the OUTPUT_FOLDER (default: './docs/')
    main()